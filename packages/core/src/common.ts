...
export function uniqName(
    name?: string | (number | string)[] | Readonly<(number | string)[]> | TemplateStringsArray,
): string {
    if (name) {
        if (typeof name === 'object' && 'raw' in name) name = name.raw;
